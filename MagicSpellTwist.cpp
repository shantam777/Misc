/*
Same problem as that in MagicSpell.cpp with a little change. Here, if a spell is used somewhere where Nitish has to pay a client, the amount will get divided by M, instead of being multiplied.
*/
#include<iostream>
#include<algorithm>
#include<functional>  
...
int payday(int pathcount,int n,int patharr[][MAX],int amount[],int k,int mul)
{
	...
	for(i=0;i<pathcount;i++)
	{
		...
		while(tempk>0)
		{
			while(j<posindex[i]&&l<negindex[i]&&tempk>0)
			{
					if((posarr[i][j]*mul+negarr[i][l])>(posarr[i][j]+negarr[i][l]/mul))
					    combarr[i]+=posarr[i][j++]*mul;
					else
						combarr[i]+=negarr[i][l++]/mul;
					tempk--;
			}
			tempk--;
		}
